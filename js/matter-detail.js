...
let tempMatterName; //saving name of selected matter

function createMattersDetails(name){
    //setting name
    name != undefined ? tempMatterName = name : '';//logic to keep matter name when navigating the doc
    //disabling matter icon
    $('#create-matter-icon-button').attr('disabled', true);
    folderLevel == 0 ? createDocumentTitle(tempMatterName) : ''; // sending name property for document title container
